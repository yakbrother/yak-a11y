...
function enhanceFailureSummary(summary, violation) {
    if (violation.id === 'region') {
        return summary.replace('Some page content is not contained by landmarks', 'Content must be inside a landmark region (use <main>, <nav>, etc. or elements with role="main", role="navigation", etc.)');
    }
    if (violation.id === 'landmark-one-main') {
        return summary.replace('Document does not have a main landmark', 'Document must have exactly one <main> element or an element with role="main"');
    }
    return summary;
}
...
async function generateReport(results, options = {}) {
    ...
    Object.entries(impactCounts)
        ...
        console.log(`${getImpactColor(impact)('■')} ${getUserFriendlyImpact(impact)}: ${count} ${count === 1 ? 'issue' : 'issues'}`);
    });
    // Detailed Violations
    console.log('\nDetailed Issues');
    console.log('━'.repeat(40));
