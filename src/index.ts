import { generateReport, type ReportOptions } from "./utils/reporter.js";
...
// Pre-import dependencies to avoid runtime resolution delays
...
async function findAstroIslands(page: Page): Promise<AstroIsland[]> {
  return await page.evaluate(() => {
    return Array.from(document.querySelectorAll("[data-astro-cid]")).map(
      (el) => ({
        cid: (el as HTMLElement).dataset.astroCid || "",
        framework: (el as HTMLElement).dataset.astroClient || "",
        html: el.outerHTML,
      }),
    );
  });
